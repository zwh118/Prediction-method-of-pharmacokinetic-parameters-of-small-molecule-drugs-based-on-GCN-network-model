...
for epoch in range(EPOCH):
  ...
  for i_batch, batch in enumerate(train_dataloader):
      ...
      knn.fit(RF_X, y1)
      RF_train_predict = np.ndarray.tolist(knn.predict(RF_X))
      ...
      for i_batch, batch in enumerate(valid_dataloader):
          ...
          knn.fit(RF_val_x, val_y1)
          RF_val_predict = np.ndarray.tolist(knn.predict(RF_val_x))
  ...
  joblib.dump(knn, f'epoch_{epoch}_knn.pkl')
