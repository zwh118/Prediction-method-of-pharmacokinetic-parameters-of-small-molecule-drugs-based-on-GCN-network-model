...
class MolDataset(Dataset):
    ...
    def atom_feature(self, m, atom_i):
        ...
        return np.array((self.one_of_k_encoding_unk(atom.GetSymbol(),  # 原子类型
...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, dropout):
        ...
        self.att1 = SelfAttention(4, 100, 100, 0.99)# dropout
...
lr = 0.000001
# lr = 0.001
model = GCN(100, 20, 0.9).cuda()  # 将模型加载到gpu上
...
for epoch in range(EPOCH):
  ...
  for i_batch, batch in enumerate(train_dataloader):
      ...
      for i_batch, batch in enumerate(valid_dataloader):
          ...
          val_y1 = y.cpu().numpy()
          val_pred = model(x, A).squeeze(-1)

          val_pred1 = val_pred.detach().cpu().numpy()
          val_Pred = []
          for i in range(len(val_pred1)):
              val_pred2 = [val_pred1[i]]
              val_Pred.append(val_pred2)


          val_mae = mean_absolute_error(val_y1, val_Pred)
          #print('val_mae:', val_mae)
          val_rmse = sqrt(mean_squared_error(val_y1, val_Pred))
