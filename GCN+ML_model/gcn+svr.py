...
svr = svm.SVR(kernel='sigmoid', C=10, gamma=0.1, epsilon=.1)  # 'rbf','linear','poly','sigmoid'
...
for epoch in range(EPOCH):
  ...
  for i_batch, batch in enumerate(train_dataloader):
      ...
      RF_train_predict = np.ndarray.tolist(svr.predict(RF_X))
      ...
      for i_batch, batch in enumerate(valid_dataloader):
          ...
          svr.fit(RF_val_x, val_y1)

          RF_val_predict = np.ndarray.tolist(svr.predict(RF_val_x))
  ...
  joblib.dump(svr, f'epoch_{epoch}_svr.pkl')
