...
XG_model = xgb.XGBRegressor(max_depth=6, learning_rate=0.5, n_estimators=100)
...
for epoch in range(EPOCH):
  ...
  for i_batch, batch in enumerate(train_dataloader):
      ...
      XG_model.fit(XG_X, y1)

      XG_train_predict = XG_model.predict(XG_X)
      XG_train_Pre = []
      for i in range(len(XG_train_predict)):
          Pred2 = [XG_train_predict[i]]
          XG_train_Pre.append(Pred2)

      #print('train_pre:', XG_train_Pre)


      a = []
      b = abs(XG_train_Pre - y1)
      #print(b)
      #print(len(b))
      ...
      mae = mean_absolute_error(y1, XG_train_Pre)
      rmse = sqrt(mean_squared_error(y1, XG_train_Pre))
      #R2 = r2_score(y1, Pred)
      MAE.append(mae)
      RMSE.append(rmse)
      ...
      for i_batch, batch in enumerate(valid_dataloader):
          ...
          XG_val_Pre = []
          for i in range(len(XG_val_predict)):
              Pred2 = [XG_val_predict[i]]
              XG_val_Pre.append(Pred2)
          #print('val_pre:', XG_val_Pre)


          val_mae = mean_absolute_error(val_y1, XG_val_Pre)
          #print('val_mae:', val_mae)
          val_rmse = sqrt(mean_squared_error(val_y1, XG_val_Pre))
  ...
  XG_model.get_booster().save_model(f'epoch_{epoch}_xgb.model')
  # XG_model.get_booster().save_model('xgb.model')
